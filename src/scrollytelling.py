...
_PART_INFO = [
    (
        ...
        "#4682B4",
        "Quem e Jesus Cristo?",
        "Stott examina tres linhas de evidencia: as declaracoes de Cristo, "
        "seu carater e sua ressurreicao.",
        ["Declaracoes", "Carater", "Ressurreicao"],
    ),
    (
        "Parte 2 - A Necessidade do Homem",
        "#DC143C",
        "O pecado e universal",
        "Tres consequencias do pecado: separacao de Deus, escravidao moral "
        "e conflito interior.",
        ["Separacao", "Escravidao", "Conflito"],
    ),
    (
        "Parte 3 - A Obra de Cristo",
        "#FF8C00",
        "A cruz: sacrificio redentor",
        "A morte de Cristo nao foi exemplo moral, mas sacrificio substitutivo. "
        "Do Antigo Testamento a cruz, o plano redentor se revela.",
        ["Perdao", "Transformacao", "Comunhao"],
    ),
    (
        ...
        "A resposta exigida: arrependimento, fe e obediencia.",
        ["Arrependimento", "Fe", "Obediencia"],
    ),
]
...
def _build_scrollytelling(analysis: BookAnalysis, groups: list[dict]) -> str:
    ...
    for i, (part_name, color, subtitle, desc, pillars) in enumerate(_PART_INFO, 1):
        part_theses = [
            t for t in analysis.theses if f"Parte {i}" in (t.part or "")
        ]
...
<title>Cristianismo Basico - John Stott | Scrollytelling</title>
<style>
{css}
</style>
</head>
...
/* Part accent bars */
.step[data-part="1"] h2 { border-left: 4px solid #4682B4; padding-left: 12px; }
.step[data-part="2"] h2 { border-left: 4px solid #DC143C; padding-left: 12px; }
.step[data-part="3"] h2 { border-left: 4px solid #FF8C00; padding-left: 12px; }
.step[data-part="4"] h2 { border-left: 4px solid #228B22; padding-left: 12px; }
...
def _build_hero(analysis: BookAnalysis) -> str:
    ...
    return f"""<section class="scroll-hero" id="hero" aria-label="Introducao">
  <h1>Cristianismo Basico</h1>
  <div class="author">John Stott</div>
  <div class="tagline">Uma jornada pelo argumento central da fe crista &mdash;
    {n_theses} teses, {n_citations} citacoes, 4 partes</div>
...
def _build_overview(analysis: BookAnalysis) -> str:
    ...
    return f"""<div class="step" data-step="1" aria-label="Visao geral">
  <h2>Visao Geral</h2>
  <p>O argumento de Stott se desdobra em 4 partes progressivas,
    construindo um caso cumulativo pela fe crista.</p>
  <p class="step-meta">{len(analysis.theses)} teses &bull;
    {len(analysis.chains)} cadeias logicas &bull;
    {biblical} citacoes biblicas &bull;
    {scholarly} citacoes academicas</p>
</div>"""
...
def _build_part_intro(
    ...
    return f"""<div class="step" data-step="{step_num}" data-part="{part_num}"
     aria-label="Parte {part_num} introducao">
  <h2>{_esc(subtitle)}</h2>
...
def _build_part_theses(
    ...
    if not items:
        items = "<p>Teses desta parte serao reveladas ao explorar os dados completos.</p>"
...
def _build_network() -> str:
    ...
    return """<div class="step" data-step="10" aria-label="Rede logica completa">
  <h2>Rede Logica Completa</h2>
  <p>As teses se conectam por cadeias logicas, formando uma rede
    argumentativa coerente. Conexoes entre partes aparecem em dourado.</p>
  <p class="step-meta">Visualizacao interativa com D3.js force-directed graph</p>
</div>"""
...
def _build_citations(groups: list[dict]) -> str:
    ...
    if not group_list:
        group_list = "<p>Grupos tematicos serao exibidos com dados completos.</p>"

    return f"""<div class="step" data-step="11" aria-label="Citacoes biblicas">
  <h2>Citacoes Biblicas</h2>
  <p>As citacoes se distribuem por livros da Biblia, agrupadas em temas teologicos.</p>
  {group_list}
  <p class="step-meta">Barras horizontais D3 por livro biblico</p>
</div>"""
...
def _build_conclusion(analysis: BookAnalysis) -> str:
    ...
    return f"""<section class="scroll-conclusion" id="conclusion" aria-label="Conclusao">
  <h2>Um Argumento Coerente</h2>
  <div class="convergence">
    <div class="conv-icon" style="background:#4682B4;">P1</div>
    <div class="conv-icon" style="background:#DC143C;">P2</div>
    <div class="conv-icon" style="background:#FF8C00;">P3</div>
    <div class="conv-icon" style="background:#228B22;">P4</div>
  </div>
  <div class="final-stats">
    <div class="final-stat"><span class="num">{n_theses}</span><span class="label">teses</span></div>
    <div class="final-stat"><span class="num">{n_citations}</span><span class="label">citacoes</span></div>
...
function renderOverview() {{
  ...
  container.innerHTML = `
    <div class="counter-grid">
      ...
      <div class="counter-box"><span class="counter-num" data-target="{n_biblical}">0</span><div class="counter-label">Citacoes Biblicas</div></div>
      <div class="counter-box"><span class="counter-num" data-target="{n_scholarly}">0</span><div class="counter-label">Citacoes Academicas</div></div>
    </div>
    <div class="part-bar">
      <div class="part-bar-seg" style="width:{pct[1]}%;background:#4682B4;" title="Parte 1"></div>
      <div class="part-bar-seg" style="width:{pct[2]}%;background:#DC143C;" title="Parte 2"></div>
      <div class="part-bar-seg" style="width:{pct[3]}%;background:#FF8C00;" title="Parte 3"></div>
      <div class="part-bar-seg" style="width:{pct[4]}%;background:#228B22;" title="Parte 4"></div>
    </div>`;
  animateCounters();
}}
...
const PART_PILLARS = {{
  1: {{ icon: "?", pillars: ["Declaracoes", "Carater", "Ressurreicao"], color: "#4682B4" }},
  2: {{ icon: "!", pillars: ["Separacao", "Escravidao", "Conflito"], color: "#DC143C" }},
  3: {{ icon: "+", pillars: ["Perdao", "Transformacao", "Comunhao"], color: "#FF8C00" }},
  4: {{ icon: "\\u2764", pillars: ["Arrependimento", "Fe", "Obediencia"], color: "#228B22" }}
}};
...
function renderPartTheses(partNum) {{
  ...
  const partTheses = THESES.filter(t => t.part && t.part.includes('Parte ' + partNum));
...
function renderNetwork() {{
  ...
  const nodes = THESES.map(t => ({{
    ...
    group: t.part ? parseInt(t.part.match(/Parte (\\d)/)?.[1] || '0') : 0
  }}));
  ...
  const groupColors = {{ 1: '#4682B4', 2: '#DC143C', 3: '#FF8C00', 4: '#228B22' }};
